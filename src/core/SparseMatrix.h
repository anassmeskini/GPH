...
template <typename REAL>
void SparseMatrix<REAL>::addRow(VectorView<REAL> row) {
   assert(all_of(row.indices, row.indices + row.size,
                 [&](size_t id) { return id < ncols; }));
...
template <typename REAL>
SparseMatrix<REAL>& SparseMatrix<REAL>::operator=(SparseMatrix<REAL>&& other) {
   ncols = other.ncols;
   nrows = other.nrows;
